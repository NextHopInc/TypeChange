...
def get_company_assets(company_details : dict): 
    #INFO This will hold all assets that contain Laptop / Desktop as their Type
    config_list = []
    server_list = []
    try:
        results = req.get(url=f'{CW_URL}company/configurations?conditions=company/id={company_details.get("id")} and status/name="Active"&pageSize=1000' , headers=CW_HEADER)
        print(results.url)
        ...
        if(results.status_code == 200 and len(results.json()) > 0):
            ...
            for item in results.json():
                #Only gathering the assets if they are equal to a desktop or laptop.... Can add server to it as well later.
                    ...
                    elif(item.get("type").get("name") == "Server"):
                        print(f"Name ==>> {item.get('name')} Type ==>> {item.get('type').get('name')}")
            ...
            if(valid == 'y' or valid == "Y"):
                return config_list , server_list
            else:
...
def change_type_name(config_list : list , server_list : list):
    ...
    try:
        ...
        for desktop in config_list:
            #Remove If statement before deploying
            
            results = req.patch(url=f'{CW_URL}company/configurations/{desktop.get("id")}/changeType', json=desktop_patch , headers=CW_HEADER)
        ...
        for server in server_list:
            ...
            results = req.patch(url=f'{CW_URL}company/configurations/{server.get("id")}/changeType', json=server_patch , headers=CW_HEADER)
            print(f"URL of Asset ==>> {results.url}")
            #Checking if the patch request was done correctly. If not then stop the code and tell user to restart program.
            if(results.status_code == 200):
                print("Change Successful")
...
if __name__ == "__main__":
    ...
    while running:
        ...
        if(type(company_details) != int):
            ...
            config_list , server_list = get_company_assets(company_details=company_details)
            if(type(config_list) != int):
            #Change the Configuration data type for the company 
                change_type_name(config_list=config_list , server_list=server_list)
            else:
